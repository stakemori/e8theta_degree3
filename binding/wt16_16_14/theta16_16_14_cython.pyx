...
cdef extern from "theta16_16_14_c.h":
    cpdef char * theta_c_16_16_14_0(int, int, int, int, int, int, int)
    cpdef char * theta_c_16_16_14_1(int, int, int, int, int, int, int)


def _theta16_16_14_cython_0_part(i_red_m):
    i_red, m = i_red_m
    ...
    sig_on()
    cdef char* c_str = theta_c_16_16_14_0(i_red, a, b, c, d, e, f)
    cdef bytes py_str
    try:
        py_str = c_str
    finally:
...
def _theta16_16_14_cython_1_part(i_red_m):
    i_red, m = i_red_m
    ...
    if max([a, b, c]) > 7:
        raise ValueError("Diagonal elements are too large.")
    sig_on()
    cdef char* c_str = theta_c_16_16_14_1(i_red, a, b, c, d, e, f)
    cdef bytes py_str
    try:
        py_str = c_str
    finally:
