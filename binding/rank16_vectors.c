...
static inline void _normalize_vec_w_sign(Rk16VecInt vec[16], size_t len)
...
static inline void _normalize_vec_wo_sign(Rk16VecInt vec[16], size_t len)
/* Similar to _normalize_vec_w_sign but only for permutations. */
{
  sort_int_vec(vec, len);
}

inline void normalize_vec_rk16_w_indices(Rk16VecInt vec[16],
...
num_of_vecs is the actual length of vecs.*/
{
  ...
  int found;
  Rk16VecInt vec[16] = {0};
  for (int i = idx_red; i < num_of_vecs; i += num_of_procs)
    {
